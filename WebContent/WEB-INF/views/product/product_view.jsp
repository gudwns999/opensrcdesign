...
<script>
    ...
    $(document).ready(function(){
        ...
        $.ajax({
                type: 'get',
                url: "/product/getOriginLicense.ajax",
                data: {"origin" : "<%=itemVO.getSeq()%>"},
		success: function(_data) {
            var license = _data.license.toString();
            var license03 = license.substring(2, 3);
            if (license03 == "0") {
                //디자인 허용.
				$("#stemBtn").removeClass("hideBtn");
            } else {
                //디자인 변경 미허용.
                var usable = false;
            }
...
//                        window.location.reload();
						//이미 눌린거면 풀고
						if($("#likeBTN").children().hasClass("btn-red") === true){
                            $("#likeBTN").children().removeClass("btn-red");
                            $("#likeBTN").children().addClass("btn-login");
							if(cnt==0){
							    $("#likeCNT").text("<%=Integer.parseInt(itemVO.getLikeCntF())-1%>");
                            }else{
                                $("#likeCNT").text("<%=Integer.parseInt(itemVO.getLikeCntF())%>");
							}
                            cnt++;
	                    }else if($("#likeBTN").children().hasClass("btn-login") === true){
                            $("#likeBTN").children().removeClass("btn-login");
                            $("#likeBTN").children().addClass("btn-red");
                            if(cnt==0){
                                $("#likeCNT").text("<%=Integer.parseInt(itemVO.getLikeCntF())+1%>");
                            }else{
                                $("#likeCNT").text("<%=Integer.parseInt(itemVO.getLikeCntF())%>");
...
<div class="modal-body" style="padding: 0px;background-color: white;overflow-x: hidden">
	...
	<div class="row">
		<div class="col-xs-12 col-sm-10" style="text-align: center">
			...
			%>
			<img src="<%=image.getFileUriL()%>" alt="<%=image.getFilename()%>" class="img-responsive mt-3" style="margin: auto"> <br>
			<%
			} else {
			%>
		...
		<div  class="hidden-xs col-xs-2 col-sm-2  col-sm-offset-10 col-xs-offset-10" style="position: fixed;background-color: rgb(250,250,250);height: 100%" >
			<div class="row">
				...
				<table class="table table-bordered">
					...
					<tr>
						<td>
							좋아요 : <span id="likeCNT"><%=itemVO.getLikeCntF()%></span>
							<div id = likeBTN>
					...
					<tr>
						<td>
							...
							<form action="/product/productStemRegi.do" method="get">
								...
								<input id="stemBtn" type="submit" value="파생디자인등록" class="btn btn-red btn-xs hideBtn">
								<% } %>
							</form>
							<div>
