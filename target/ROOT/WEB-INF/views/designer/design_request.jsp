...
$(function(){
	...
	listView = new ListView({
		...
		,htmlElementNoData: $('<p class="none" style="line-height: 300px;"></p>')
	});
...
<style>
	.table {margin-bottom:0px; background-color:white;}
	.request-list{clear:both; padding-left:0px;
	}
	.request-list > li{ list-style-type:none;position:relative; margin:0px 0px 0px 0px; font-size:15px; color:#414141;font-family: "Helvetica Neue", Helvetica, Arial, sans-serif}
	.request-list > li:last-child{margin:0 0 2px;}
	.request-list dt{overflow:hidden; min-height:25px; padding:20px 360px 20px 10px; cursor:pointer; background:#fff;}
	.request-list dt .txt{float:left;}
	.request-list dd{display:none; margin:4px 0 0; font-size:18px; background:#fff;}
	.request-list dd .txt{padding:30px 30px;}
	.request-list dl{margin:0px 0px 3px 0px;}
	.request-list .reply-area{padding:0 30px 20px;}
	.request-list .reply-area textarea{width:100%; height:56px; box-sizing:border-box; margin:0 0 10px; padding:10px; font-size:16px; border:1px solid darkred;}
	.request-list .reply-area li{position:relative; width:90%;padding:0 80px 15px 10px; font-size:16px; color:darkred;}
	.request-list .reply-area .name{display:inline-block; width:65px; font-weight:700; color:black;}
	.request-list .reply-area .date{position:absolute; top:0; right:15px; font-weight:700; color:black;}
	.request-list .info{position:absolute;top:20px; right:55px;}
	.request-list .info .replay{margin:0 80px 0 0;}
	.request-list li.active{border:1px solid darkred;}
	.request-list li.active dt{color:darkred;}
	...
	.request-list .btn-set .btn-modi{background:#767676 no-repeat 8px 7px;}
	.request-list .btn-set .btn-del{background:red no-repeat 9px 7px;}

	.custom-select{display:inline;}

	.btn-cmmt{float:right; display:inline}
...
<div class="wrap">
	...
	<div class="container">
		...
		<div class="container maintitle mb-5" >
			...
			<span class="maintitle2"> <%=memberDivString%> 의뢰</span>
		</div>
		<table class="table table-striped">
			<thead>
			<tr>
				<th style="width: 71.5%">제목</th>
				<th style="width: 8%">댓글 수</th>
				<th>날짜</th>
				<th>글쓴이</th>
			</tr>
			</thead>
		</table>
		<ul id="listViewId" class="request-list" > <!-- 게시판 -->
		</ul>
...
<!--				<div class="custom-select">
			...
			<a href="javascript:goBoardAddView();" class="btn btn-red" style="float:right">새 글 쓰기</a>
